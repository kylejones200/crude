...
use crate::normalize::normalize_raw_assay;
...
pub fn import_assay_bytes(bytes: &[u8], extension: &str) -> AssayResult<Crude> {
    ...
    let raw = match ext.as_str() {
        ...
        "pdf" => {
            let raw = crate::pdf::parse_pdf(bytes)?;
            return normalize_raw_assay(raw);
        }
        other => return Err(AssayError::UnsupportedFormat(other.to_string())),
    };

    normalize_raw_assay(raw)
}
...
fn parse_excel(bytes: &[u8]) -> AssayResult<RawAssay> {
    ...
    for row in range.rows() {
        ...
        let value = &row[1];
        map_excel_field(&mut raw, &key, value);
    }
...
fn map_excel_field(raw: &mut RawAssay, key: &str, value: &calamine::Data) {
    use calamine::Data;
